...
```do loop``` выполняет ```N``` итераций в зависимости от двух значений на вершине стека (n1 n2);
```N = n1 - n2```. Внутри конструкции ```do loop``` возможно использовать
переменную ```i```, ```i = n2 + <номер итерации>```.
Аналогично ```for (i = n2; i < n1; i++) {}```

```begin until``` выполняет итерации пока на вершине стека
по достижению конструкции until не будет лежать истинное значение (!= 0).

Оператор ```variable``` выделяет в памяти переменную (взаимодействие с помощью ```! @``` , ```allot``` выделяет в памяти
память для хранения данных с
...
- идет снизу вверх, представляет собой адресное пространство от 00 до 15000, от 00 до 511 хранятся строковые литералы,
...
Реализуется функцией [translator.py:split_to_terms](translator.py#L62).
...
Правила трансляции Term в Opcode реализованы в [translator.py:term_to_opcodes](translator.py#L366).
...
DataPath реализован в [machine.py:DataPath](machine.py#L129)

Stack Controller является частью DataPath, включен в
класс [machine.py:DataPath](machine.py#L129)

ControlUnit реализован в [machine.py:ControlUnit](machine.py#L250)
...
Реализация дешифрации команд [machine.py:ControlUnit:decode_and_execute_instruction](machine.py#L318).
